...
-- Includes: share-link + OTP access, verified-user board sessions, user_id-based vote dedup
...
-- ─── QnA Posts ───────────────────────────────────────────────────────────────
-- share_code: auto-generated 9-char code for public share links
-- join_enabled: admins can toggle whether the share link is active
...
CREATE UNIQUE INDEX idx_qna_posts_share_code
  ON qna_posts(share_code) WHERE share_code IS NOT NULL;
...
-- One session per board per verified user. Users must exist in the users table.
-- OTP is identity verification only — no guest users, no anonymous accounts.
-- session_token: 32-byte hex (256-bit entropy), used as X-Session-Token header.
-- is_anonymous: controls display name only; user_id is always stored for dedup/moderation.
...
CREATE UNIQUE INDEX idx_user_sessions_board_user ON user_sessions(qna_id, user_id);
...
-- ─── OTPs ────────────────────────────────────────────────────────────────────
-- Shared by admin login and public board join flows
CREATE TABLE otps (
  id         UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  email      TEXT NOT NULL,
...
-- ─── Atomic like toggle (JWT users — user_id path) ───────────────────────────
...
BEGIN
  ...
  ELSE
    INSERT INTO question_likes (question_id, user_id)
      VALUES (p_question_id, p_user_id) ON CONFLICT DO NOTHING;
