...
export default function PublicQuestionCard({ question, boardId, isClosed, isNew, session, onUpdate, onDelete }) {
  ...
  return (
    <div className={`px-6 py-4 group transition-opacity duration-150${answered ? ' opacity-75' : ''}${isNew ? ' animate-[fadeSlideIn_200ms_ease_out]' : ''}`}>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 pr-2">
          ...
          <div className="flex items-center gap-0 mb-1 flex-wrap">
            <span className="text-sm font-semibold text-slate-700 leading-snug">{question.author_name || 'Anonymous'}</span>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150"
                >
                  {saving ? 'Saving…' : 'Save'}
                </button>
                <button
        ...
        {/* Action pill */}
        {!editing && (
          <div className="inline-flex items-center rounded-full border border-slate-200 bg-white shadow-sm overflow-hidden shrink-0">
            {isOwner && !question._isOptimistic && (
              <div className="max-w-0 group-hover:max-w-[52px] overflow-hidden transition-all duration-200 ease-out inline-flex items-center">
                <button
                  ref={menuBtnRef}
                  type="button"
                  onClick={openMenu}
            ...
            <button
              ...
              aria-pressed={voted}
              className={[
                'flex items-center gap-1.5 pl-2.5 pr-3 py-1.5 text-xs font-semibold transition-colors duration-150 select-none',
                isClosed || question._isOptimistic
                  ? 'text-slate-300 cursor-default'
                  : voted
                  ? 'text-indigo-600 hover:bg-slate-50'
                  : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700 active:bg-slate-100',
              ].join(' ')}
            >
