  ...
  return (
    <div className="inline-flex items-center rounded-full border border-slate-200 bg-white shadow-sm overflow-hidden shrink-0">
      ...
      {(showAdminActions || showMore) && (
        <div className="max-w-0 group-hover:max-w-[160px] overflow-hidden transition-all duration-200 ease-out inline-flex items-center">
          {showAdminActions && (
            <>
              {/* Mark handled (answered in Slack) */}
              <button
                type="button"
          ...
          {/* separator between hover-reveal and vote — clipped when collapsed */}
          <div className="w-px h-5 bg-slate-200 shrink-0" />
        </div>
      )}
      ...
      {/* ── Vote ── */}
      {isAdmin ? (
        <div className="flex items-center gap-1.5 pl-2.5 pr-3 py-1.5 text-xs font-semibold text-slate-400 select-none cursor-default">
