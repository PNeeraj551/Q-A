...
export function QuestionActionPill({
  ...
  return (
    <>
      {/* Desktop (≥md): Hover-revealed action group inspired by Slack & Linear */}
      <div className="hidden md:inline-flex items-center rounded-lg border border-slate-200/80 bg-white shadow-sm shadow-slate-100/50 overflow-hidden shrink-0 transition-all duration-200 ease-out select-none">
            ...
            {showAdminActions && (
              <>
                ...
                <button
                  ...
                  className={[
                    'px-2.5 py-2 transition-colors duration-150 disabled:cursor-not-allowed focus:outline-none focus:bg-slate-50',
                    pushedToSlack
                ...
                {showMore && <div className="w-px h-4 bg-slate-100 shrink-0" />}
              </>
            )}
        ...
        {/* Vote — Desktop (inspired by Slido's minimal look) */}
        {isAdmin ? (
          <div className={[
            'inline-flex items-center gap-1.5 px-3 py-2 text-xs select-none cursor-default font-medium',
            isTopQuestion ? 'text-indigo-600' : count > 0 ? 'text-blue-500' : 'text-slate-400',
          ].join(' ')}>
            {voteContent}
          </div>
        ) : (
      ...
      {/* Mobile (<md): Touch-friendly pill layout with large tap targets */}
