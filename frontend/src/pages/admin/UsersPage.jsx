...
import { getUsers, createUser, updateUser, unlockUser, deleteUser } from '../../api/userManagement'
...
// ─── Security badge — only renders for exceptional states ─────────────────────
function SecurityBadge({ status }) {
  if (status === 'LOCKED') {
    return (
      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-semibold tracking-wide bg-amber-50 text-amber-700 border border-amber-200 select-none">
        <svg className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.25" viewBox="0 0 24 24">
...
// ─── Kebab menu ───────────────────────────────────────────────────────────────
function ActionMenu({ user, onEdit, onUnlock, onDelete }) {
  ...
  return (
    <>
      <button
        ...
        className="w-7 h-7 flex items-center justify-center rounded text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors duration-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
        aria-label="Actions"
      >
        ...
        >
          ...
          {user.status === 'LOCKED' && (
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 text-emerald-700 hover:bg-emerald-50 transition-colors text-left"
            >
...
function SkeletonRow() {
  return (
    <tr className="border-b border-slate-100 last:border-0">
      ...
      <td className="px-4 py-3"><div className="h-3 bg-slate-100 rounded animate-pulse w-44" /></td>
      <td className="px-4 py-3"><div className="h-5 bg-slate-100 rounded animate-pulse w-14" /></td>
      <td className="px-4 py-3"><div className="h-3 bg-slate-100 rounded animate-pulse w-20" /></td>
      <td className="px-4 py-3"><div className="h-3 bg-slate-100 rounded animate-pulse w-14" /></td>
      <td className="px-4 py-3" />
    </tr>
  )
}
...
export default function UsersPage() {
  ...
  return (
    <DashboardLayout
      title="Users"
      subtitle={`${users.length} member${users.length !== 1 ? 's' : ''}${lockedCount > 0 ? ` · ${lockedCount} locked` : ''}`}
      actions={
        <button
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/80">
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-56">Name</th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Email</th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-24">Role</th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-28">Joined</th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-28">Security</th>
                ...
                {loading ? (
                  ...
                  users.map((user) => (
                    ...
                    >
                      ...
                      {/* Email */}
                      <td className="px-4 py-3 text-slate-500 font-mono text-xs tracking-tight">
                        {user.email}
                      </td>
                      ...
                      {/* Security state */}
                      <td className="px-4 py-3">
                        <SecurityBadge status={user.status || 'VERIFIED'} />
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {editingId === user.id ? (
                          ...
                          <span className="text-xs text-slate-400">Unlocking…</span>
                        ) : !user.is_root ? (
          ...
          {!loading && users.length > 0 && (
            <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400">
                {lockedCount > 0
                  ? <span className="text-amber-600 font-medium">{lockedCount} locked</span>
                  : 'All accounts normal'
                }
              </span>
            </div>
